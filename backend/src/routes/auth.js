  ...
  try {
    const result = await resend.emails.send({
      ...
      html: `<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;">
        ...
        <a href="${verifyUrl}" style="display:inline-block;padding:10px 20px;background:#0d6efd;color:#fff;text-decoration:none;border-radius:4px;">Verify Email</a>
        <p style="margin-top:16px;color:#666;font-size:13px;">Or copy: <a href="${verifyUrl}">${verifyUrl}</a></p>
      </div>`
    });
...
router.post('/register', async (req, res) => {
  ...
  if (password.length === 0) return res.status(400).json({ error: 'Password cannot be empty.' });
  try {
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Login successful.', token, user: { id: user.id, name: user.name, email: user.email, status: user.status } });
  } catch (err) {
...
router.get('/verify-email', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE users SET status = CASE WHEN status = 'blocked' THEN 'blocked' ELSE 'active' END, verification_token = NULL
       WHERE verification_token = $1 RETURNING id, name, email, status`,
      [token]
    );
    if (result.rows.length === 0) return res.status(400).json({ error: 'Invalid or already used verification token.' });
